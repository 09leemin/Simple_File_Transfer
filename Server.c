...
int main(int argc, char **argv)
{
    ...
    char command[5], filename[MAXLINE], dir[MAXLINE], buf[BUFSIZ];
    ...
    if (listen_sock == -1)  errquit("tcp_listen fail");
    ...
    while(1) {
        ...
        if(!strcmp(command, "cd\n")) {              /* cd 명령어 처리(경로변경) */
            ...
            while (nsize != fsize) {    /* 256씩 전송 */
                int fpsize = fread(buf, 1, BUFSIZ, file);
                nsize += fpsize;
            ...
            nbyte = BUFSIZ;
            while(nbyte >= BUFSIZ) {
                nbyte = recv(accp_sock, buf, BUFSIZ, 0);            /* 256씩 수신 */
                success = fwrite(buf, sizeof(char), nbyte, file);   /* 파일 쓰기 */
                if (nbyte < BUFSIZ) success = 1;
            }
            send(accp_sock, &success, sizeof(int), 0);              /* 성공 여부 전송 */
            fclose(file);
        }
